...
- 0.42 menu continue + visual lab (human playtest): Continue Campaign button on the main menu (hidden with no active campaign, routes through game-over when appropriate) plus a lab game-over guard; artifact portrait thumbnails and staff faces across lab rows (locked/dead dimmed), hire faces on candidates. New MENULAB suite. Suites 30/30.
- 0.41 result-screen text cutoff fix (human playtest): SKILL BREAKDOWN box was 60px with scrolling disabled, hiding 5 of 7 lines. Root fix is fit_content on all 7 clipped RichTextLabels across result/artifact/selection screens (same defect class, audited repo-wide); result screen added to screenshot coverage. Suites 29/29.
- 0.40 critical experiment-screen fix (found by human playtest): scientist clicks wrote the global index while Run/Queue read the local one, so the run button never armed — the core loop was dead from the UI (probes bypass UI, which is why automation never caught it). Plus toggle-mode highlights and metadata-based experiment highlighting (locked-row filtering had shifted button indices). New UI_OK interaction suite drives the real handlers, verified to fail 5 ways on the old code. Suites 29/29.
- 0.39 widescreen + store shots: stretch aspect expand (no more letterboxing; all screens re-verified at 16:9), store/ committed (8 capsules/logo + 7 screenshots at exact 1920x1080). Suites 28/28, no sim changes.
- 0.38 store-ready art: transparent logo in repo + on main menu (fixed a latent menu overflow — Card was centering 1010px of content in 768px, cutting the top on small windows; menu now scrolls); full Steam set built and verified (header/main/library/hero/vertical/small at spec, 231px legibility passes). Store files staged outside repo for upload. Suites 28/28.
- 0.37 art round two: infirmary ward image, capsule backdrop on game-over screen, flask app icon (project + export .ico); re-exported (109MB exe + 6.4MB pck with art), smoke exit 0. Suites 28/28.
- 0.36 generated art integrated: 72 sliced assets (18 artifact portraits, 12 staff/director portraits, places, incidents, 32 icons, keyart/logo/hero) wired into artifact/scientist/intel/menu/infirmary/facilities screens with null-safe fallbacks; shader portraits retained underneath. Suites 28/28, no sim changes.
- 0.35 first shippable binary: export templates installed, Windows x86_64 preset committed (tests excluded from pck), release export succeeds (109MB exe + 470KB pck), headless smoke test passes (menu boots, 180 frames, exit 0). Release checklist refreshed. Suites 28/28.
- 0.34 thin-playstyle verdicts: expert-systems 0/3 (Expert wall holds except batch — the lone past win was leak-era pollution), hard-skilled 1/4 → 4/4 after gating probe ops behind a $4000 buffer (early all-in op spending was probe recklessness, not game balance; funded ops sweep Hard). No game-code changes. Suites 28/28 (unchanged).
- 0.33 multi-seed confidence + init hygiene: pillar playstyles now probe 2-3 seeds (normal-pure 3/3, hard-pure 0/2, normal-systems 3/3, expert-batch 3/3); closed cross-campaign leaks found by reordering probes (helios progress, incident cooldown, offer day, artifact selection never reset — second campaigns in a session started dirty), plus SIM reset test and a const-aliasing fix in save-load. Honest residual: ~2 probe lines still move with order; read pillars, not lines. Suites 28/28.
- 0.32 system badges: Signature Style (5 signature runs), Mentor (5 trainings, new counter with save/load), Clean Sweep (counter-intel sweep) — 20 badges, Steam mirror extended, N8 drift guard holds. Probe bit-identical to baseline. Suites 28/28.
- 0.31 counter-sweep + endgame + stats: OP_COUNTER now sweeps active player sabotage (flavor + S1 asserts); SCN_ENDGAME cold-starts Act 3 (war chest, hot rivals, endgame moves fire via scenario path); game-over screen shows a run-stats line (experiments/incidents/crises/tech/deals, N9-tested); corrected the brief's op count. Probe table bit-identical to 0.30 baseline — determinism holds across rounds. Suites 28/28.
- 0.30 mutators + true determinism: MUT_APPRENTICE (training half-price) and MUT_AUDIT (crisis payouts double) join daily/weekly rotation with H3 hook tests; closed a real determinism leak (unseeded Array.shuffle in world-event scheduling explained all historical single-seed wobble) with seeded Fisher-Yates + SIM schedule-identity test — two full probe runs now diff zero. Baseline: Easy/Normal sweep, Hard pure LOSE / systems 2/4 / skilled splits, Expert batch+systems WIN. Suites 28/28.
- 0.29 specialist + mild coverage + recovery training: SCN_SPECIALIST (J002 at 20 knowledge, signature day one) via new knowledge_set key; 3 mild J016-18 incidents (minor only, no crises) + 2 contracts (27-deck); training grants +1 influence in recovery. Probe pillars hold with healthy margins. Suites 28/28.
- 0.28 staff training sink: train physics/engineering/observation/curiosity at 400+15/level, capped 95, each session costing a full day (+8 stress, +2 XP) via train buttons on the scientist screen; codex + tour mention it. Probe: Easy/Normal sweep, Hard pure LOSE / systems 3/4 / skilled splits / batch+signature WIN, Expert batch d25. Suites 28/28.
- 0.27 signature experiments + codex: all 18 artifacts have a unique run (gain 5-8, $500-1200, knowledge gates 15-35, 4 marked DANGEROUS via the existing equipment-failure path, skill-matched), artifact-locked in is_experiment_unlocked with other-artifact rows hidden in selection UI; signature probe playstyle proves the needle moves (normal d23 vs pure d34, hard-signature WINs where generic pure loses). Codex rewritten to 0.27 reality + SIGNATURE RUNS and STAFF & LOYALTY sections. Suites 28/28.
